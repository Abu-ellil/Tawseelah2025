...
import { View, ScrollView, TextInput, TouchableOpacity, Alert, Image } from 'react-native';
...
// Mock data
import { mockDriverRegister } from '../../utils/mockData';
...
const RegisterScreen = () => {
  ...
  const handleRegister = async () => {
    if (!name || !email || !phone || !password || !confirmPassword || !licenseNumber || !carModel) {
    ...
    try {
      // في تطبيق حقيقي، سيتم استدعاء API لتسجيل السائق
      const response = await mockDriverRegister(userData);
    ...
    <ScrollView className="flex-1" style={{ backgroundColor: colors.background }}>
      <View className="px-6 pt-10 pb-6">
        ...
        <View className="items-center mb-6">
          ...
          />
          <Text className="text-2xl font-bold" style={{ color: colors.primary }}>توصيلة - السائق</Text>
        ...
        <View>
          ...
          <Text className="text-lg font-medium mb-2" style={{ color: colors.text }}>رقم الرخصة</Text>
          <TextInput
            className="p-4 rounded-xl mb-4 border"
            style={{ 
              backgroundColor: colors.card,
          ...
          <Text className="text-lg font-medium mb-2" style={{ color: colors.text }}>نوع السيارة</Text>
          <TextInput
            className="p-4 rounded-xl mb-4 border"
            style={{ 
              backgroundColor: colors.card,
