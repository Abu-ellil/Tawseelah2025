...
import { fetchStoresStart, fetchStoresSuccess } from '../store/slices/storeSlice';
...
const HomeScreen = () => {
  ...
  const renderStore = ({ item }) => (
    ...
    >
      ...
      <View className="p-3">
        ...
        <Text className="text-xs" style={{ color: colors.placeholder }}>{item.address}</Text>
      </View>
    </TouchableOpacity>
  );
  ...
  const renderProduct = ({ item }) => (
    <TouchableOpacity 
      ...
      onPress={() => {
        ...
        navigation.navigate('StoreDetails', { storeId: item.store, scrollToProduct: item._id });
      }}
    >
      <Image 
      ...
      <View className="p-2">
        ...
        <Text className="text-xs" style={{ color: colors.placeholder }} numberOfLines={1}>{item.category}</Text>
        <Text className="text-sm font-bold" style={{ color: colors.primary }}>{item.price} ر.س</Text>
      </View>
    </TouchableOpacity>
  );
  ...
  return (
    <ScrollView className="flex-1" style={{ backgroundColor: colors.background }}>
      {/* مرحباً بالمستخدم */}
      <View className="p-4">
        <Text className="text-2xl font-bold" style={{ color: colors.text }}>مرحباً بك في توصيلة</Text>
        <Text style={{ color: colors.placeholder }}>ما الذي تبحث عنه اليوم؟</Text>
      </View>
      ...
      <View className="px-4 mb-4">
        ...
        >
          <Text className="mr-3" style={{ color: colors.placeholder }}>ابحث عن منتجات أو متاجر...</Text>
      ...
      <View className="mb-6">
        <View className="px-4 py-2 flex-row justify-between items-center">
          <Text className="text-xl font-bold" style={{ color: colors.text }}>المتاجر القريبة</Text>
      ...
      <View className="mb-6">
        <View className="px-4 py-2 flex-row justify-between items-center">
          <Text className="text-xl font-bold" style={{ color: colors.text }}>المنتجات المميزة</Text>
      ...
      <View className="px-4 py-6">
        <Text className="text-xl font-bold mb-4" style={{ color: colors.text }}>التصنيفات</Text>
        <View className="flex-row flex-wrap justify-between">
          {['إلكترونيات', 'ملابس', 'مأكولات', 'أثاث', 'كتب', 'ألعاب'].map((category, index) => (
            <TouchableOpacity 
              key={index}
              ...
              onPress={() => {
                ...
                dispatch({ type: 'products/filterProductsByCategory', payload: category });
                navigation.navigate('Products');
              }}
            >
              <Text className="font-medium" style={{ color: colors.text }}>{category}</Text>
            </TouchableOpacity>
          ))}
        </View>
      </View>
