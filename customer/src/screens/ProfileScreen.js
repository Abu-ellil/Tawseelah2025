...
const ProfileScreen = () => {
 ...
 const dispatch = useDispatch();
  ...
  const settings = [
    ...
    { id: 2, title: 'عناويني', icon: '📍', action: () => navigation.navigate('Addresses') },
    { id: 3, title: 'طرق الدفع', icon: '💳', action: () => navigation.navigate('PaymentMethods') },
    { id: 4, title: 'التقييمات', icon: '⭐', action: () => navigation.navigate('Reviews') },
    { id: 5, title: 'الإشعارات', icon: '🔔', action: () => navigation.navigate('Notifications') },
    { id: 6, title: 'الخصوصية', icon: '🔒', action: () => navigation.navigate('Privacy') },
    { id: 7, title: 'مساعدة', icon: '❓', action: () => navigation.navigate('Help') },
    { id: 8, title: 'من نحن', icon: 'ℹ️', action: () => navigation.navigate('About') },
  ];
      ...
      <View className="p-6" style={{ backgroundColor: colors.primary }}>
        <View className="flex-row items-center">
          ...
          <View className="mr-4 flex-1">
            <Text className="text-xl font-bold text-white">{user?.name || 'المستخدم'}</Text>
