...
import { searchProducts, fetchProductsSuccess } from '../store/slices/productSlice';
    ...
    if (activeTab === 'products') {
      return (
        ...
        >
          ...
          <View className="flex-1 mr-3">
            <Text className="font-medium" style={{ color: colors.text }} numberOfLines={1}>{item.name}</Text>
            <Text className="text-sm" style={{ color: colors.placeholder }} numberOfLines={1}>{item.category}</Text>
            <Text className="text-base font-bold mt-1" style={{ color: colors.primary }}>{item.price} ر.س</Text>
          </View>
        </TouchableOpacity>
      );
    } else {
      return (
        ...
        >
          ...
          <View className="flex-1 mr-3">
            <Text className="font-medium" style={{ color: colors.text }} numberOfLines={1}>{item.name}</Text>
            <Text className="text-sm" style={{ color: colors.placeholder }} numberOfLines={1}>{item.category || 'متجر عام'}</Text>
            <View className="flex-row items-center mt-1">
              <Text className="text-sm" style={{ color: colors.text }}>⭐ {item.rating}</Text>
              <Text className="text-xs mx-2" style={{ color: colors.placeholder }}>({item.ratingCount || 0} تقييم)</Text>
            </View>
          </View>
        </TouchableOpacity>
      );
  ...
  return (
    <SafeAreaView className="flex-1" style={{ backgroundColor: colors.background }}>
      ...
      <View className="flex-row justify-around p-2">
        <TouchableOpacity 
          className={`flex-1 items-center py-3 ${activeTab === 'products' ? 'border-b-2' : ''}`}
          style={{ borderColor: activeTab === 'products' ? colors.primary : 'transparent' }}
          onPress={() => setActiveTab('products')}
        >
          <Text style={{ 
            color: activeTab === 'products' ? colors.primary : colors.placeholder,
        ...
        <TouchableOpacity 
          className={`flex-1 items-center py-3 ${activeTab === 'stores' ? 'border-b-2' : ''}`}
          style={{ borderColor: activeTab === 'stores' ? colors.primary : 'transparent' }}
          onPress={() => setActiveTab('stores')}
        >
          <Text style={{ 
            color: activeTab === 'stores' ? colors.primary : colors.placeholder,
        ...
        ListEmptyComponent={
          searchQuery ? (
            <View className="items-center justify-center py-20">
              <Text style={{ color: colors.text }}>لا توجد نتائج لـ "{searchQuery}"</Text>
            </View>
          ) : (
            <View className="items-center justify-center py-20">
      ...
      {searchQuery === '' && (
        <View className="px-4 mt-4">
          <Text className="font-bold mb-3" style={{ color: colors.text }}>البحث الشائع</Text>
          <View className="flex-row flex-wrap">
            {['هاتف', 'مطعم', 'فستان', 'حاسوب', 'حلويات'].map((suggestion, index) => (
              <TouchableOpacity 
                key={index}
